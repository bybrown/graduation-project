% IIR filter data flow representation.
function DFfilterCreator(order, filterName, nominator, denominator)
  n = order + 1;
  ...
  for i = 1:2*n - 2
    if i == 1
      conn = "Node.connect(x, 1, S1, 1, 0);" + newline;
      filter = filter + conn;
      
      conn = "Node.connect(S3, 1, S1, 2, 0);" + newline;
      filter = filter + conn;
    elseif i == 2*n - 2
      count = 2*n - 2;
      node = "S" + num2str(count);
      
      conn = "Node.connect(M"+num2str(count)+", 1, "+node+", 1, 0);" + newline;
      filter = filter + conn;
      
      conn = "Node.connect(M"+num2str(count + 2)+", 1, "+node+", 2, 0);" + newline;
      filter = filter + conn;
    elseif  i == 2*n - 3
      count = 2*n - 3;
      node = "S" + num2str(count);
      ...
      conn = "Node.connect(M"+num2str(i)+", 1, "+node+", 1, 0);" + newline;
      filter = filter + conn;
      
      conn = "Node.connect(S"+num2str(i + 2)+", 1, "+node+", 2, 0);" + newline;
      filter = filter + conn;
    end
  ...
  for i = 2:2*n
    if mod(i,2) == 0
      ...
      conn = "Node.connect(S1, 1, "+node+", 1, "+delay+");" + newline;
      filter = filter + conn;
      
      conn = "Node.connect(b"+delay+", 1, "+node+", 2, 0);" + newline;
      filter = filter + conn;
    else
      delay = num2str((i - 1) / 2);
      ...
      conn = "Node.connect(a"+delay+", 1, "+node+", 2, 0);" + newline;
      filter = filter + conn;
    end
  ...
  filter = filter + "Node.connect(S2, 1, y, 1, 0);" + newline;
